...
func Throttle(name string) httpcontract.Middleware {
	return func(ctx httpcontract.Context) {
		if limiter := ginpackage.RateLimiterFacade.Limiter(name); limiter != nil {
			if limits := limiter(ctx); len(limits) > 0 {
				for _, limit := range limits {
					if instance, ok := limit.(*httplimit.Limit); ok {
						...
						if ginpackage.CacheFacade.Has(timer) {
							value := ginpackage.CacheFacade.GetInt(key, 0)
							if value >= instance.MaxAttempts {
								expireSecond := ginpackage.CacheFacade.GetInt(timer, 0) + instance.DecayMinutes*60
								...
								var err error
								if currentTimes, err = ginpackage.CacheFacade.Increment(key); err != nil {
									panic(err)
								}
							}
						} else {
							...
							err := ginpackage.CacheFacade.Put(timer, carbon.Now().Timestamp(), expireMinute)
							if err != nil {
								panic(err)
							}

							err = ginpackage.CacheFacade.Put(key, currentTimes, expireMinute)
							if err != nil {
								panic(err)
							}
						}
...
func key(ctx httpcontract.Context, limiter string, limit *httplimit.Limit) (string, string) {
	...
	prefix := ginpackage.ConfigFacade.GetString("cache.prefix")
	if len(limit.Key) == 0 {
